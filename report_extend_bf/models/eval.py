...
def genshi_lookup_attr(cls, obj, key):
    ...
    try:
        ...
        if hasattr(obj.__class__, key):
            ...
            try:
                if type(obj) == dict:
                    # If data values (report_manager.summary break line in report)
                    # Example: .report_action([], data={'lines': lines, 'summary': report_manager.summary, 'email_subject': report_manager.email_subject})
                    if type(obj[key]) == str:
                        val = format_multiline_value(obj[key])
                    else:
                        val = obj[key]
                else:
                            ...
                            if field_name == key:
                                if field.get('type') in ['selection', 'date', 'datetime', 'text', 'html', 'monetary', 'boolean', 'many2many', 'float', 'char']:
                                    ...
                                    if field.get('type') == 'selection':
                                        ...
                                        val = format_multiline_value(html2plaintext(field_val).strip())
